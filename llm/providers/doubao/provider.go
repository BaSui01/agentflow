...
// DoubaoProvider implements ByteDance Doubao (豆包) LLM Provider.
// Doubao uses OpenAI-compatible API format via Volcano Engine (火山方舟).
...
func (p *DoubaoProvider) HealthCheck(ctx context.Context) (*llm.HealthStatus, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return &llm.HealthStatus{Healthy: false, Latency: latency}, fmt.Errorf("doubao health check failed: status=%d msg=%s", resp.StatusCode, msg)
	}
...
func (p *DoubaoProvider) buildHeaders(req *http.Request, apiKey string) {
...
func (p *DoubaoProvider) Completion(ctx context.Context, req *llm.ChatRequest) (*llm.ChatResponse, error) {
	...
	body := openAIRequest{
		Model:       providers.ChooseModel(req, p.cfg.Model, "Doubao-1.5-pro-32k"),
	...
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, fmt.Sprintf("%s/api/v3/chat/completions", strings.TrimRight(p.cfg.BaseURL, "/")), bytes.NewReader(payload))
	if err != nil {
		return nil, fmt.Errorf("failed to create request: %w", err)
	}
...
func (p *DoubaoProvider) Stream(ctx context.Context, req *llm.ChatRequest) (<-chan llm.StreamChunk, error) {
	rewrittenReq, err := p.rewriterChain.Execute(ctx, req)
	if err != nil {
		return nil, &llm.Error{
			Code:       llm.ErrInvalidRequest,
