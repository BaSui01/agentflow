...
func TestOpenAIProvider_GenerateVideo_Success(t *testing.T) {
	callCount := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodPost && r.URL.Path == "/v1/videos/generations" {
			w.Header().Set("Content-Type", "application/json")
			require.NoError(t, json.NewEncoder(w).Encode(map[string]string{"id": "vid-123", "status": "pending"}))
			return
		}
		if r.Method == http.MethodGet && r.URL.Path == "/v1/videos/generations/vid-123" {
			...
			if callCount >= 2 {
				require.NoError(t, json.NewEncoder(w).Encode(map[string]any{
					"id": "vid-123", "status": "completed", "created_at": 1700000000,
					"data": []map[string]string{{"url": "https://example.com/video.mp4"}},
				}))
			} else {
				require.NoError(t, json.NewEncoder(w).Encode(map[string]string{"id": "vid-123", "status": "processing"}))
			}
			return
		}
	...
	p := newTestProvider(t, server.URL, false)
	resp, err := p.GenerateVideo(context.Background(), &llm.VideoGenerationRequest{Model: "sora", Prompt: "a sunset"})
	require.NoError(t, err)
	require.NotNil(t, resp)
...
func TestOpenAIProvider_GenerateVideo_UpstreamError(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte(`{"error":{"message":"Invalid prompt"}}`))
	}))
...
func TestOpenAIProvider_GenerateVideo_PollTimeout(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodPost {
			w.Header().Set("Content-Type", "application/json")
			require.NoError(t, json.NewEncoder(w).Encode(map[string]string{"id": "vid-timeout", "status": "pending"}))
			return
		}
		w.Header().Set("Content-Type", "application/json")
		require.NoError(t, json.NewEncoder(w).Encode(map[string]string{"id": "vid-timeout", "status": "processing"}))
	}))
	t.Cleanup(server.Close)
