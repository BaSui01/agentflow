...
func TestBaseCapabilityProvider_PostJSONDecode(t *testing.T) {
	...
	p := providerbase.NewBaseCapabilityProvider(providerbase.CapabilityConfig{
		Name:    "test",
		BaseURL: srv.URL,
		APIKey:  "test-key",
	})
...
func TestChooseCapabilityModel(t *testing.T) {
	if m := providerbase.ChooseCapabilityModel("req-model", "default"); m != "req-model" {
		t.Errorf("expected req-model, got %s", m)
	}
	if m := providerbase.ChooseCapabilityModel("", "default"); m != "default" {
