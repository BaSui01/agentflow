...
// InitDatabase initializes database schema for multi-provider support
...
func InitDatabase(db *gorm.DB) error {
	...
	// Create indexes (GORM handles this automatically via struct tags)
	// But we can add custom indexes if needed
	
	return nil
}

// SeedExampleData seeds example data with 50+ mainstream models (2026)
...
func SeedExampleData(db *gorm.DB) error {
	...
	// Seed 50+ mainstream models (2026 latest)
	models := []LLMModel{
		// OpenAI (5 models)
	...
	// Seed provider-model mappings with 2026 pricing and context windows
	...
	// Seed API keys (example - replace with real keys)
