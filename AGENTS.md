...
- **重构先读计划**：遇到 `重构`、`cleanup`、`refactor`、`deslop`、`架构治理`、`边界收敛`、`计划归档` 等任务，先阅读 `AGENTS.md`、`docs/重构计划/` 下当前仍生效的文档、`docs/architecture/startup-composition.md` 与相关 ADR；需要历史上下文时，再补读 `docs/重构计划/归档/`。
- **当前基线优先**：`docs/重构计划/` 根目录是当前执行基线，`docs/重构计划/归档/` 仅作历史参考；不得用归档计划覆盖当前活动计划。
- **先对齐再建议**：给出重构方案前，先对齐相关计划中的目标、Phase/DoD、剩余 `[ ]`、涉及模块、架构约束与守卫命令，再输出重构建议。
- **建议必须具体**：重构建议必须尽量落到明确动作，例如“删除并行入口 / 下沉到 usecase / 合并重复实现 / 收口到 Builder/Factory/Registry / 补守卫或补 ADR”，避免只给空泛原则。
- **直接实施也不能跳过阅读**：即使用户要求直接改代码，也要先完成计划阅读与简短建议输出，然后继续执行，不要跳过计划对齐。
- **缺计划先补方案**：如果相关重构没有现成计划，或现有计划明显过期/失真，先补一个最小可执行计划或修订建议，再进入实现。
- **归档与收尾分开判定**：单个计划满足 `todo = 0`、不再作为当前执行基线、且已修正直接引用时，可判定“该计划可归档”；只有 `python scripts/refactor_plan_guard.py gate` 通过时，才允许判定“仓库整体可停止/可收尾”。
- **普通实现不滥用计划治理**：纯 bugfix、纯功能实现、与重构计划或架构治理无关的修改，不强制走重构计划治理流程。

### 10) 重构相关 Skills / 工作流选择（建议）

- **计划治理/推进/归档/验收**：优先使用 `cn-refactor-plan-governance`。
- **架构审计/分层治理/模块拆并**：优先使用 `architecture-refactor-zh`。
- **清理重复实现/去样板/降复杂度**：优先使用 `ai-slop-cleaner`。
- **需要先出方案与步骤拆解**：优先使用 `plan` 或 `ralplan`。
- **需要实施后复审/代码质量回看**：优先使用 `code-review-workflow` 或 `code-review`。
- **技能使用原则**：只选择完成当前任务所需的最小技能集合；技能用于增强分析与执行，不得替代实际代码阅读、活动计划对齐、守卫校验与测试验证。
