...
type HybridRetrievalConfig struct {
	...
	RerankAlpha  float64 `json:"rerank_alpha"` // rerank 与原始分数的加权系数，默认 0.7
...
func (r *HybridRetriever) IndexDocuments(docs []Document) error {
	return r.indexDocuments(context.Background(), docs)
}

// AddDocument incrementally indexes a single document without replacing the existing in-memory corpus.
func (r *HybridRetriever) AddDocument(ctx context.Context, doc Document) error {
	return r.indexDocuments(ctx, []Document{doc})
}

func (r *HybridRetriever) indexDocuments(ctx context.Context, docs []Document) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	r.documents = mergeIndexedDocuments(r.documents, docs)
	r.buildDocIndex()
	...
	if r.vectorStore != nil && r.config.UseVector {
		if err := r.vectorStore.AddDocuments(ctx, docs); err != nil {
			// 回滚 BM25 统计到之前的状态
...
func mergeIndexedDocuments(existing []Document, incoming []Document) []Document {
	if len(incoming) == 0 {
		return existing
	}
	merged := make([]Document, len(existing))
	...
	for _, doc := range incoming {
		if idx, ok := index[doc.ID]; ok {
			merged[idx] = doc
			continue
		}
		index[doc.ID] = len(merged)
