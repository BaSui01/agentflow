...
func (p *GrokProvider) HealthCheck(ctx context.Context) (*llm.HealthStatus, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return &llm.HealthStatus{Healthy: false, Latency: latency}, fmt.Errorf("grok health check failed: status=%d msg=%s", resp.StatusCode, msg)
	}
...
func (p *GrokProvider) buildHeaders(req *http.Request, apiKey string) {
