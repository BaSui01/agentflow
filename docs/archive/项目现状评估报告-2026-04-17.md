...
> **⚠️ 历史快照说明（已补当前入口说明）**
>
> 本文档主体形成于 2026-04-17，部分路径与覆盖率口径早于 2026-04-22 的 Agent 模块收口。
> 当前 Agent 正式入口应以以下文档为准：
>
...
> - `github.com/BaSui01/agentflow/agent` 根包已删除，不再作为公开入口
...
### 3.1 核心架构
- **`BaseAgent`**（当前主实现位于 `agent/execution/runtime/base_agent.go` + `agent/execution/runtime/request_runtime.go`）：涵盖 LLM Gateway、工具管理、事件总线、上下文工程、Guardrails、Checkpoint、持久化与推理模式注册表。
...
- **构建器模式**：`agent/execution/runtime.Builder` 是当前正式 runtime 构造入口；`AgentBuilder` 仅保留为高级扩展路径。
...
| 团队适配 | **中** | `agent/adapters/teamadapter` 覆盖率 58.1%，适配逻辑较薄 |
...
2. **并发控制已优化**（当前实现位于 `agent/execution/runtime/base_agent.go`）
...
- **agent/adapters/teamadapter**：58.1%
- **`go test ./agent/...` 全部通过**
...
| Agent 层 | ~81% | `agent/adapters/teamadapter` 58.1%，`agent/integration/hosted` 61.4% |
...
### 剩余优化项
- `agent/adapters/teamadapter`、`rag/runtime` 的覆盖率仍有提升空间
- 长链路工作流建议上线前补充压测
...
| `agent/execution/runtime/base_agent.go` | BaseAgent 基础结构、生命周期与 Observe |
| `agent/execution/runtime/request_runtime.go` | ChatCompletion / StreamCompletion / Plan / Execute 主路径 |
| `agent/execution/runtime/execution.go` | Sandbox 执行器（`Run()` 已对接真实 `os/exec`） |
| `agent/capabilities/memory/coordinator.go` | 记忆协调器 |
| `agent/execution/context/assembler.go` | 上下文组装与预算裁剪 |
