...
### 3.1 核心架构
- **`BaseAgent`**（当前主实现位于 `agent/runtime/base_agent.go` + `agent/runtime/request_runtime.go`）：涵盖 LLM Gateway、工具管理、事件总线、上下文工程、Guardrails、Checkpoint、持久化与推理模式注册表。
...
- **构建器模式**：`agent/runtime.Builder` 是当前正式 runtime 构造入口；`AgentBuilder` 仅保留为高级扩展路径。
...
2. **并发控制已优化**（当前实现位于 `agent/runtime/base_agent.go`）
...
| `agent/runtime/base_agent.go` | BaseAgent 基础结构、生命周期与 Observe |
| `agent/runtime/request_runtime.go` | ChatCompletion / StreamCompletion / Plan / Execute 主路径 |
| `agent/runtime/execution.go` | Sandbox 执行器（`Run()` 已对接真实 `os/exec`） |
