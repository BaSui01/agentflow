...
- [x] 2026-03-25：回填默认执行链与运行时装配收敛：`BaseAgent.Execute(...)` 现统一走配置驱动的扩展管线（tool selection / prompt enhancer / skills / enhanced memory / observability）后再进入唯一核心执行；动态工具选择结果通过 execution-scoped `RunConfig.ToolWhitelist` 真实影响请求工具白名单；`CachingResolver` 新增 shared enhanced memory 注入并由 `WireMongoRuntimeStores(...)` 接入 Mongo 增强记忆；`internal/app/bootstrap/RegisterDefaultRuntimeAgentFactory(...)` 改为覆盖 `generic` 默认工厂，使 HTTP 运行时默认创建的 agent 直接带 runtime 特性；`api/handlers/multimodal.go` 的 API→领域消息转换补齐 `images/videos/reasoning/refusal/annotations` 透传；通过 `go test ./agent/... ./api/... ./internal/app/bootstrap/... ./cmd/agentflow/...`。
- [x] 2026-03-25：完成本轮架构守卫对齐：将 `agent/session_manager.go` 合并入 `agent/steering.go`、`agent/run_config.go` 合并入 `agent/request.go`，恢复 `agent` 根包生产文件预算到 `20`；同步 `scripts/arch_guard.ps1` 的单文件 `pkg/` allowlist，删除失效的 `middleware` 项并补录 `jsonschema`，保持与 `architecture_guard_test.go` 一致；通过 `go test ./agent/... ./api/... ./internal/app/bootstrap/... ./cmd/agentflow/...` 与 `scripts/arch_guard.ps1`。
- [x] 2026-04-17：完成 Agent LLM 统一入口收口：`BaseAgent` 显式暴露 `MainGateway()/ToolGateway()`，`runtime.Builder -> NewDefaultReasoningRegistry -> agent/reasoning/*` 默认链路全部改为依赖 `llmcore.Gateway`；`TreeOfThought/ReWOO/PlanAndExecute/DynamicPlanner/IterativeDeepening/Reflexion` 已删除业务层 `provider.Completion/Stream` 直调，并由 `agent/reasoning/gateway_chat.go` 统一解包 `UnifiedResponse.Output -> *llm.ChatResponse`；回归通过 `go test ./agent/reasoning/... ./agent/runtime/... ./agent/evaluation/... ./agent/deliberation/... ./agent/structured/... ./api/handlers/... ./internal/app/bootstrap/... ./cmd/agentflow/...`。
- [x] 2026-04-17：完成 Agent 统一构建主链补收口：`agent.AgentRegistry` 内置 typed factory 不再直调 `NewBaseAgent(...)`，改为统一复用 `agent.NewAgentBuilder(...).Build()`；`agent/multiagent/default_modes.go` 的 hierarchical synthetic base 也改为通过同一 builder 构造，避免 `registry/multiagent` 保留隐藏并行入口；新增 `architecture_guard_test.go::TestAgentUnifiedBuilderEntryPoints` 防回退，并补充 `agent/registry_test.go` 验证 typed factory 仍保留默认 system prompt/skill categories。
