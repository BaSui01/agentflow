...
- [x] 2026-04-17：完成 workflow 对共享 gateway 的启动链收口：`internal/app/bootstrap/workflow_step_dependencies_builder.go` 已将 `WorkflowRuntimeOptions.LLMProvider` 替换为 `LLMGateway llmcore.Gateway`，`cmd/agentflow/server_handlers_runtime.go` 与 `server_hotreload.go` 均改为向 workflow runtime 透传共享 `llmRuntime.Gateway`；新增架构守卫禁止 `workflow/` 非测试文件直调 `.Completion(` / `.Stream(`；回归通过 `go test ./workflow/... ./internal/app/bootstrap/... ./cmd/agentflow/... ./api/handlers/...`。
