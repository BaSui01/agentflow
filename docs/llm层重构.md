...
- [x] 2026-03-02：完成 Phase-4 第1项（入口统一 token 预算预估）：`llm/gateway/gateway.go` 在 `preflightPolicy` 中新增统一 token 估算链路（优先 metadata，缺失时按 capability 自动估算，默认复用 `llm/tokenizer.GetTokenizerOrEstimator`，并回填 `metadata.estimated_tokens`）；覆盖 `chat/embedding/rerank/moderation/tools` 预估路径；新增 `llm/gateway/gateway_policy_estimation_test.go` 验证预算拦截与 metadata 覆盖语义。
