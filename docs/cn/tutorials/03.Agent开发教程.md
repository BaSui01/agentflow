...
> 流式执行时，`agent.RuntimeStreamEvent` 还会附带 `sdk_event_type` / `sdk_event_name`，用于对齐 OpenAI Agents SDK 的语义流事件模型。当前已对齐的事件包括 `raw_response_event` 以及运行项级别的 `message_output_created`、`tool_called`、`tool_output`。
