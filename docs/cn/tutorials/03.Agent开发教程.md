...
```go
agent := agent.NewBaseAgent(agent.AgentConfig{
    ID:    "my-agent",
    Model: "claude-opus-4-20250115",  // 贵模型：最终内容生成
    Tools: []string{"web_search", "calculator"},
}, logger)

// 设置主 Provider（贵模型）
agent.SetProvider(claudeProvider)

// 设置工具调用 Provider（便宜模型）
agent.SetToolProvider(gpt4oMiniProvider)
```
