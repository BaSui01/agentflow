...
// 使用 Builder 构建 Agent
myAgent, err := agent.NewAgentBuilder(types.AgentConfig{
...
agent.GlobalRegistry.Register("custom_analyzer", func(
    ...
    // 创建自定义 Agent（Builder 模式）
    baseAgent, err := agent.NewAgentBuilder(config).
        WithProvider(provider).
        WithMemory(memory).
...
在 `AgentConfig` 中设置 `toolProvider` 字段：

```go
ag, _ := agent.NewAgentBuilder(types.AgentConfig{
    Core: types.CoreConfig{ID: "my-agent"},
    LLM:  types.LLMConfig{Model: "claude-opus-4-20250115"},
    Runtime: types.RuntimeConfig{
        Tools: []string{"web_search", "calculator"},
    },
}).WithProvider(claudeProvider).WithLogger(logger).Build()
