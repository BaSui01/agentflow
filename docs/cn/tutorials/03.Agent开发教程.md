...
### 使用 `agent/execution/runtime.Builder`
...
如果你希望由仓库统一装配 Agent、Workflow、RAG，再对外暴露稳定入口，请优先使用 `sdk.New(opts).Build(ctx)`；本教程聚焦 `agent` 子模块，因此默认示例使用 `agent/execution/runtime.Builder`。
...
新的多 Agent 接入统一推荐使用 `agent/collaboration/team`：

```go
import "github.com/BaSui01/agentflow/agent/collaboration/team"
...
- 常规接入仍应优先走 `sdk.New(opts).Build(ctx)` 或 `agent/execution/runtime.Builder`。
...
12. **多 Agent 优先用 `agent/collaboration/team`**：`hierarchical`、`collaboration`、`crews` 仅作为 legacy/advanced 兼容 surface
...
推荐仍通过 `agent/execution/runtime.Builder` 先完成主 runtime 装配，再设置专用 `toolProvider`：
...
> 源码位置：`agent/completion.go:66-69`
...
原 `agent/base.go` 中的核心执行逻辑已拆分为两个独立文件，职责更清晰：

| 文件 | 职责 | 核心方法 |
|------|------|----------|
| `agent/completion.go` | LLM 调用与流式处理 | `ChatCompletion()`, `StreamCompletion()` |
| `agent/react.go` | ReAct 循环与执行 | `Plan()`, `Execute()`, `Observe()` |
| `agent/base.go` | Agent 基础结构 | 配置、状态管理、生命周期 |

- `completion.go`：处理双模型架构、上下文工程优化、流式 ReAct 事件发射
- `react.go`：实现 Plan/Execute/Observe 三阶段循环，集成 Guardrails 输入/输出验证
