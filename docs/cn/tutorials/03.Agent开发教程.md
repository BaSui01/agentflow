...
AgentFlow 支持双模型架构：使用便宜模型（如 GPT-4o-mini）做工具调用，贵模型（如 Claude Opus）做最终内容生成，大幅降低成本。
...
1. 如果有可用工具且设置了 `toolProvider`，ReAct 循环使用 `toolProvider`
2. 如果没有设置 `toolProvider`，退化为使用主 `provider`
3. 无工具场景直接使用主 `provider` 生成内容
