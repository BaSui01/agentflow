...
myAgent, err := runtime.NewBuilder(provider, logger).WithOptions(opts).Build(ctx, types.AgentConfig{
...
runtime.NewBuilder(provider, logger).
    WithOptions(opts).
    Build(ctx, cfg)
```
...
```go
ag, _ := runtime.NewBuilder(provider, logger).Build(ctx, types.AgentConfig{
    Core: types.CoreConfig{ID: "my-agent"},
    Runtime: types.RuntimeConfig{
