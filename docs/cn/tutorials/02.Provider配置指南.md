...
这条链路适用于框架内置的 `provider + api_key pool` 场景，不是新的 channel-based routing 主入口。
...
- `MultiProviderRouter` 继续保留，但定位为 legacy DB-backed provider routing
- `MultiProviderRouter` 与 `ChannelRoutedProvider` 是 `Gateway` 后两个互斥入口：前者服务内置 DB-backed `provider + api_key pool`，后者服务部部注入的 channel-based routing；一次请求不要串双路由
...
4. 老部署可继续使用 `MultiProviderRouter`，直到新的 channel-based adapter 完成验证。

详细设计与 phased migration 见 `docs/architecture/channel-routing-extension.md`。

## API Key 池化

`llm/runtime/router.APIKeyPool` 是按 Provider 维度的、数据库驱动的 API Key 池。通常你只需要调用 `MultiProviderRouter.InitAPIKeyPools(...)` 初始化并加载所有 Provider 的 Key 池；如果需要也可直接使用：
