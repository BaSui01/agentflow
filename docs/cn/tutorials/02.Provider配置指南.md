...
- `MultiProviderRouter` 与 `ChannelRoutedProvider` 是 `Gateway` 后两个互斥入口：前者服务内置 DB-backed `provider + api_key pool`，后者服务外部注入的 channel-based routing；一次请求不要串双路由
...
- 内置 server 启动链已经支持 `llm.main_provider_mode`；外部项目可以通过 `llm/runtime/compose.RegisterMainProviderBuilder(...)` 注册 `channel_routed` builder，或直接复用 `channelstore.NewMainProviderBuilder(...)`
