...
> 说明：兼容厂商（如 DeepSeek / Qwen / GLM / Grok / MiniMax / Mistral / Doubao）在 chat 主链上统一通过 `vendor.NewChatProviderFromConfig(...)` 接入；厂商特有的 image/video/audio/fine-tuning/context cache 等能力仍保留在各自能力实现中，不要求与 chat 构造入口绑定。
