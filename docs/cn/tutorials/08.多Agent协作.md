...
其余 `hierarchical`、`collaboration`、`crews` surface 继续保留，但定位为 legacy/advanced 兼容层，不再作为默认新项目方案。

## Legacy：层级 Agent (Supervisor-Worker)
...
在 MultiAgentSystem 中通过 `MultiAgentConfig.SharedState` 注入。
