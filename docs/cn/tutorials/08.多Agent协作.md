...
`agent/collaboration/team` 是 AgentFlow 的官方多 Agent facade。新接入默认只推荐这条路径：
...
```go
team := teamadapter.NewCollaborationTeam("my-team", agents, "debate", logger)
result, err := team.Execute(ctx, "分析这个问题", agent.WithMaxRounds(3))
```

支持三种适配器：
- `NewCollaborationTeam` — 协作模式
- `NewHierarchicalTeam` — 分层模式
- `NewCrewTeam` — 角色组模式
