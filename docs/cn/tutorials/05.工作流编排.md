...
1. 节点输入/输出：将需要传播的状态字段作为节点输出，并在后续节点中消费。
2. 检查点与执行历史：对长流程使用 Checkpoint + History 持久化，支持恢复与审计。
...
`workflow/circuit_breaker.go` 提供 DAG 节点级熔断保护，防止故障节点拖垮整个工作流。
...
// 使用默认配置
config := workflow.DefaultCircuitBreakerConfig()
// FailureThreshold:           5     // 连续失败 5 次触发熔断
// RecoveryTimeout:            30s   // 熔断后 30 秒进入半开状态
// HalfOpenMaxProbes:          3     // 半开状态最多 3 个探测请求
// SuccessThresholdInHalfOpen: 2     // 半开状态连续成功 2 次恢复
...
```go
// 创建熔断器注册表（管理所有节点的熔断器）
registry := workflow.NewCircuitBreakerRegistry(config, eventHandler, logger)

// 获取或创建节点的熔断器
cb := registry.GetOrCreate("node-1")

// 检查是否允许请求
allowed, err := cb.AllowRequest()
if !allowed {
    // 熔断中，跳过执行
    log.Println("circuit open:", err)
    return
}
...
```go
// 实现 CircuitBreakerEventHandler 接口
type MyHandler struct{}

func (h *MyHandler) OnStateChange(event workflow.CircuitBreakerEvent) {
    log.Printf("节点 %s: %s → %s (原因: %s, 失败次数: %d)",
        event.NodeID, event.OldState, event.NewState, event.Reason, event.Failures)
}
```
...
`workflow/dsl/` 提供声明式 YAML 工作流定义，支持变量插值、条件分支、循环和子图。
...
```go
import "github.com/BaSui01/agentflow/workflow/dsl"

// 创建解析器
parser := dsl.NewParser()

// 注册自定义步骤工厂
parser.RegisterStep("my_step", func(config map[string]interface{}) (workflow.Step, error) {
    return &MyCustomStep{Config: config}, nil
})

// 注册命名条件
parser.RegisterCondition("is_valid", func(ctx context.Context, data interface{}) (bool, error) {
    return data != nil, nil
})

// 从文件解析
wf, err := parser.ParseFile("workflow.yaml")
...
验证器检查：
- 必填字段（version, name, entry）
- 节点 ID 唯一性
- 引用完整性（step、agent、tool、变量引用）
- 节点类型合法性（action, condition, loop, parallel, subgraph, checkpoint）
- 条件节点必须有 on_true 或 on_false
- 循环节点必须有循环定义
