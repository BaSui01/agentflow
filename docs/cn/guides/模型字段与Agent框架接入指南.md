...
> 官方核验：截至 2026-05-02（本轮先核验请求参数口径，完整模型列表快照后续继续补齐）。
> 参数矩阵：主流供应商请求字段与 AgentFlow 主面映射见 [./模型供应商请求参数矩阵.md](./模型供应商请求参数矩阵.md)。
...
- 基础 web search 配置
- approval policy / sandbox mode 一类 runtime execution policy
- memory external-context policy（MCP / web search / 外部工具参与时的 recall / writeback 开关；当前已真实接入 `RecallForPrompt(...)` 与 `ObserveTurn(...)` 行为边界）
- subagent / handoff execution policy 的首版统一表达
...
`types.ToolProtocolOptions` 当前除原有工具协议字段外，还已经进入：
...
- `AllowHandoffs` 已能通过 `RunConfig.SubagentAllowHandoffs` 继续下沉到 `HandoffAdapter.Execute(...)`
- `MaxParallelism` 已接入 `CollectParallelResults(...)` 的真实 fan-out 并发限制
- `MaxDepth` 已接入 `prepareSubagentContext(...)` / `ExecuteWithSubagents(...)` 的真实深度拦截
- `RealtimeCoordinator.CoordinateSubagents(...)` 也已对齐同一套深度/并发限制
- 这条链路已经能通过 `RunConfig` / `RunConfigFromInputContext(...)` / `ResolveRunConfig(...)` 自动桥接到 runtime
- `AgentExecuteRequest.Context` 经过 `applyAgentRoutingContext(...)` 也会把 `subagent_allow_handoffs` / `subagent_max_depth` / `subagent_max_parallelism` 注入 `RunConfig`
- `workflow/steps/buildOrchestrationAgentInput(...)` 也已开始系统性注入 `subagent_max_depth` / `subagent_max_parallelism`

这意味着：

> AgentFlow 当前已经不只是“能做 handoff / 多 agent”，而是开始把多 agent 执行策略本身正式收口到 `Tools` 面，而不是继续散落在 team mode 或 runtime helper 的私有逻辑里。
...
| Gemini `speechConfig` / `imageConfig` / `safetySettings` | `types.ModelOptions` -> `types.ChatRequest` -> Gemini `GenerateContentConfig` | 已接入正式主面；`SafetySettings`、`OutputSpeech`、`OutputImage` 由 adapter 统一降级到 provider 边界 |
...
> **正式主面已经覆盖低层 DTO 中主要的通用模型请求字段、三大 native provider（OpenAI / Gemini / Anthropic）的 thinking 控制字段，以及 Gemini safety / speech / image 的首版结构化输出控制。**
...
> 2026-05-02 实现补齐：`agent/adapters.DefaultChatRequestAdapter` 之前已经能把 `Model.Verbosity` / `Model.Phase` 降级到 `types.ChatRequest`；本轮补上 `llm/providers/openai` 的 Responses API provider 映射，使 `verbosity` 真正进入 `text.verbosity`，`phase` 作为 Responses extra field 透传。
...
| `text.verbosity` | `Model.Verbosity` -> `types.ChatRequest.Verbosity` -> OpenAI Responses `text.verbosity` | 已接入正式主面与 OpenAI Responses provider |
...
| `phase` | `Model.Phase` -> `types.ChatRequest.Phase` -> OpenAI Responses extra field `phase` | 已接入正式主面与 OpenAI Responses provider |

### 当前结论

不要把 OpenAI Responses 的原始 request struct 暴露到 `agent/runtime`。当前已采用统一语义：`Model.Verbosity`、`Model.Phase`、`Model.PreviousResponseID`、`Model.ConversationID`、`Model.Include`、`Model.Truncation`、`Model.Store`、`Model.ServiceTier`，由边界层翻译为 OpenAI Responses 参数。
...
| `thinking.type` / adaptive vs enabled | `Model.ThinkingType` -> `types.ChatRequest.ThinkingType` -> Anthropic thinking config | 已接入正式主面；`ThinkingType` 优先于 legacy `ReasoningMode` |
| `budget_tokens` / `effort` | `Model.ReasoningEffort` / `Model.ThinkingBudget` | 已有统一字段；provider 边界按 Claude 模型能力决定是否映射为 effort 或 budget |
...
- 请求侧：已用 `Model.ThinkingType` / `Model.ReasoningEffort` / `Model.ThinkingBudget` 承载主面语义；后续如需更复杂组合再抽 `ThinkingOptions`
...
| `thinkingBudget` / `thinkingLevel` / `includeThoughts` | `Model.ThinkingBudget` / `Model.ThinkingLevel` / `Model.IncludeThoughts` | 已接入正式主面 |
| `responseModalities` | `Model.Modalities` | 已接入通用输出模态字段；Gemini 专有映射仍在 provider 边界补齐 |
| `speechConfig` / `imageConfig` / `mediaResolution` / `safetySettings` | `Model.OutputSpeech` / `Model.OutputImage` / `Model.MediaResolution` / `Model.SafetySettings` | 已接入正式主面；Gemini provider 映射到 `SpeechConfig` / `ImageConfig` / `SafetySettings` |
...
Gemini 不要继续靠 provider 内部“偷渡字段”长期扩展。当前正式主面已经补齐首版：
...
这样 `llm/providers/gemini` 只负责把统一语义翻译成 `GenerationConfig` / `SpeechConfig` / `ImageConfig` / `SafetySettings`，而不是自己维护另一套 runtime API。
...
这会比今天的 `vendor.Profile.LanguageModels map[string]string` 更适合作为“最新模型数据”的代码事实来源。当前实现已提供零依赖目录结构和查找能力，并在 `types.DefaultModelCatalog()` / `types.DefaultModelDescriptors()` 中沉淀了 2026-05-02 首版内置快照；同时提供 `types.LoadModelCatalogJSON` / `types.LoadModelCatalogFile`，启动层可通过 `llm.model_catalog_path` 或 `AGENTFLOW_LLM_MODEL_CATALOG_PATH` 让 `internal/app/bootstrap.BuildModelCatalog` 加载外部 JSON 快照，并经 `agent/runtime.CachingResolver.WithDefaultProvider` / `WithModelCatalog` 把默认 agent 模型别名规范化为 canonical model id。`types.RequiredModelCapabilities` / `types.ValidateModelCapabilities` 已把 `ModelDescriptor.Capabilities` 用于 agent builder 边界校验：当请求 tools、structured output、reasoning/thinking 或 web search，而 catalog 已知模型未声明对应能力时，会提前返回输入校验错误，避免在 provider 层散落硬编码或运行到远端才失败。
...
第三轮继续补（Codex CLI execution semantics）：

- `Control.ApprovalPolicy`
- `Control.SandboxMode`
- `Control.MemoryExternalContext`
- `Tools.Subagents`
- hosted tool / MCP tool authorization metadata 注入 `approval_policy` / `sandbox_mode` / `memory_external_context_policy`
- team runtime 首版 handoff policy：`agent.WithTeamAllowHandoffs(false)`
- async subagent fan-out：`RunConfig` -> `ExecutionOptions.Tools.Subagents` -> `ExecuteWithSubagents(...)`
- realtime coordinator：`RunConfig` -> `ExecutionOptions.Tools.Subagents` -> `CoordinateSubagents(...)`
- handoff adapter：`RunConfig.SubagentAllowHandoffs` / `subagent_max_depth` -> `HandoffAdapter.Execute(...)`
- request entrypoint：`AgentExecuteRequest.Context` -> `applyAgentRoutingContext(...)` -> `RunConfig`
- orchestration 输入桥接：`buildOrchestrationAgentInput(...)` 会把 `subagent_max_depth` / `subagent_max_parallelism` 注入 `agent.Input.Context`
- memory deeper policy：`SearchLongTerm(...)`、`StartConsolidation(...)`、`ConsolidateOnce(...)`

后续仍需补的 provider-native 深水区字段：

- Gemini safety/media：官方枚举、模型级约束、组合限制与错误信息归一化
