...
Note: Google Gemini Developer API `POST /v1beta/models/{model}:generateContent` and `POST /v1beta/models/{model}:streamGenerateContent` are now registered as inbound HTTP compatibility endpoints (unified into the `ChatService -> llm/gateway` chain). Vertex AI paths such as `POST /v1/projects/{project}/locations/{location}/publishers/google/models/{model}:generateContent` remain provider outbound protocols owned by `llm/providers/gemini` / `llm/providers/vendor`.
