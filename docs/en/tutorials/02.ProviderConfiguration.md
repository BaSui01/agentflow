...
This path is for the built-in `provider + api_key pool` runtime. It is not the recommended main entry for the newer channel-based routing architecture.
...
- `MultiProviderRouter` and `ChannelRoutedProvider` are the two mutually exclusive entries behind `Gateway`: the former serves the built-in DB-backed `provider + api_key pool` path, the latter serves externally injected channel-based routing. Do not stack them in one request chain.
...
4. Existing deployments can stay on `MultiProviderRouter` until the new channel adapters are validated.

See `docs/architecture/channel-routing-extension.md` for the detailed architecture and phased migration plan.

## API Key Pooling

`llm/runtime/router.APIKeyPool` is a per-provider, database-backed API key pool. `MultiProviderRouter.InitAPIKeyPools(...)` creates and loads pools automatically, but you can use it directly if needed:
