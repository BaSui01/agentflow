...
- `agent.NewAgentBuilder`, `agent.CreateAgent`, and `agent.BuildBaseAgent` remain available only for advanced extension or low-level customization
...
If you are wiring only the `agent` submodule runtime, use `agent/execution/runtime.Builder` directly:
