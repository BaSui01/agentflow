...
### Using `agent/execution/runtime.Builder`
...
If you want one repository-level runtime that assembles Agent, Workflow, and RAG together, prefer `sdk.New(opts).Build(ctx)`. This tutorial focuses on the `agent` submodule, so the examples default to `agent/execution/runtime.Builder`.
...
New integrations should prefer `agent/collaboration/team` as the only official multi-agent surface.
...
The remaining surfaces below are retained as legacy or advanced compatibility layers.

## Legacy Hierarchical Agent

Supervisor-Worker pattern for historical hierarchical agent systems.
...
Historical collaboration patterns for multi-agent systems.
...
- Normal integration should still start from `sdk.New(opts).Build(ctx)` or `agent/execution/runtime.Builder`.
...
12. **Use `agent/collaboration/team` for multi-agent work**: Treat hierarchical/collaboration/crews as legacy or advanced compatibility surfaces
