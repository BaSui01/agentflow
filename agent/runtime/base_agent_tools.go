...
func (b *BaseAgent) Observe(ctx context.Context, feedback *Feedback) error {
	...
	case b.memory != nil:
		...
		if err := b.SaveMemory(ctx, feedback.Content, MemorySemantic, metadata); err != nil {
...
func (b *BaseAgent) collectContextMemory(ctx context.Context, values map[string]any) []string {
	...
	if b.memoryFacade != nil {
		...
		for _, item := range b.memoryFacade.LoadContext(ctx, b.ID()) {
