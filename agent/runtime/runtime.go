...
type EventType = legacy.EventType
type Event = legacy.Event
type EventHandler = legacy.EventHandler
...
type ExecutionMode = legacy.ExecutionMode
type Language = legacy.Language
type SandboxConfig = legacy.SandboxConfig
type ExecutionRequest = legacy.ExecutionRequest
type ExecutionResult = legacy.ExecutionResult
type SandboxExecutor = legacy.SandboxExecutor
type RealProcessBackend = legacy.RealProcessBackend
type HostedAdapter = legacy.HostedAdapter
type Checkpoint = legacy.Checkpoint
type CheckpointMessage = legacy.CheckpointMessage
type CheckpointToolCall = legacy.CheckpointToolCall
type ExecutionContext = legacy.ExecutionContext
...
const (
	...
	ModeDocker  = legacy.ModeDocker
	ModeProcess = legacy.ModeProcess
	ModeWASM    = legacy.ModeWASM
	ModeNative  = legacy.ModeNative

	LangPython     = legacy.LangPython
	LangJavaScript = legacy.LangJavaScript
	LangTypeScript = legacy.LangTypeScript
	LangGo         = legacy.LangGo
	LangRust       = legacy.LangRust
	LangBash       = legacy.LangBash
)

var (
	NewBuilder                          = legacy.NewBuilder
	NewAgentRegistry                    = legacy.NewAgentRegistry
	NewCachingResolver                  = legacy.NewCachingResolver
	NewCheckpointManagerFromNativeStore = legacy.NewCheckpointManagerFromNativeStore
	DefaultSandboxConfig                = legacy.DefaultSandboxConfig
	NewSandboxExecutor                  = legacy.NewSandboxExecutor
	NewRealProcessBackend               = legacy.NewRealProcessBackend
	NewHostedAdapter                    = legacy.NewHostedAdapter
