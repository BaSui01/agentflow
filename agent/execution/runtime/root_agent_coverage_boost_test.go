...
func TestSkillInstructionsContext(t *testing.T) {
	ctx := agentcontext.WithSkillInstructions(context.Background(), []string{"use tool X"})
	instructions := agentcontext.SkillInstructionsFromContext(ctx)
...
func TestSkillInstructionsContext_Missing(t *testing.T) {
	instructions := agentcontext.SkillInstructionsFromContext(context.Background())
...
func TestMemoryContext(t *testing.T) {
	ctx := agentcontext.WithMemoryContext(context.Background(), []string{"mem1", "mem2"})
	result := agentcontext.MemoryContextFromContext(ctx)
...
func TestMemoryContext_Missing(t *testing.T) {
	result := agentcontext.MemoryContextFromContext(context.Background())
	if len(result) != 0 {
...
func TestDecideTraceFeedbackMode(t *testing.T) {
	...
	mode := planner.Plan(&agentcontext.TraceFeedbackPlanningInput{
		AgentID:          "agent-1",
		TraceID:          "trace-1",
		SessionID:        "session-1",
		UserInputContext: complexInput.Context,
	...
	if mode.RecommendedAction != agentcontext.TraceFeedbackSynopsisAndHistory {
	...
	mode = planner.Plan(&agentcontext.TraceFeedbackPlanningInput{
		...
		TraceID:          "trace-2",
		SessionID:        "session-1",
		UserInputContext: simpleInput.Context,
	...
	if mode.RecommendedAction != agentcontext.TraceFeedbackSkip {
		t.Fatalf("expected skip action, got %#v", mode.RecommendedAction)
	}
	if len(mode.SuppressedLayers) == 0 || !assert.Contains(t, mode.SuppressedLayers, "memory_recall") {
...
func TestComposedTraceFeedbackPlanner_HintAdapterForceSkip(t *testing.T) {
	...
	plan := planner.Plan(&agentcontext.TraceFeedbackPlanningInput{
		AgentID:          "agent-1",
		TraceID:          "trace-1",
		SessionID:        "session-1",
		UserInputContext: map[string]any{"trace_feedback_force_skip": true},
		Signals: TraceFeedbackSignals{
			HasPriorSynopsis:     true,
			HasCompressedHistory: true,
			Resume:               true,
		},
		Snapshot: agentcontext.ExplainabilitySynopsisSnapshot{
			Synopsis:          "ended=validation_failed",
	...
	if plan.RecommendedAction != agentcontext.TraceFeedbackSkip {
...
func TestRecordTraceFeedbackDecision(t *testing.T) {
	...
	ag.recordTraceFeedbackDecision("trace-1", TraceFeedbackPlan{
		...
		RecommendedAction:  agentcontext.TraceFeedbackSynopsisOnly,
...
func TestScopedPersistenceStores_ScopedID_Empty(t *testing.T) {
	...
	if agentpersistence.ScopedID(s.Scope(), "") != "" {
		t.Fatal("expected empty for empty id")
	}
	if agentpersistence.ScopedID(s.Scope(), "abc") != "tenant1/abc" {
		t.Fatalf("expected 'tenant1/abc', got '%s'", agentpersistence.ScopedID(s.Scope(), "abc"))
	}
}
