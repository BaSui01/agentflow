...
// DiscoveryRegistryAdapter adapts tools.DiscoveryService to the
// DiscoveryRegistry interface used by the federation bridge.
type DiscoveryRegistryAdapter struct {
	service *tools.DiscoveryService
}

// NewDiscoveryRegistryAdapter creates a new adapter wrapping the given
// DiscoveryService.
func NewDiscoveryRegistryAdapter(service *tools.DiscoveryService) *DiscoveryRegistryAdapter {
	return &DiscoveryRegistryAdapter{service: service}
}

// RegisterAgent converts an AgentRegistration to tools.AgentInfo and
...
func (a *DiscoveryRegistryAdapter) RegisterAgent(ctx context.Context, info *AgentRegistration) error {
	...
	agentInfo := tools.AgentInfoFromCard(card, false)
	agentInfo.Endpoint = info.Endpoint

	// Use Card.Name as the agent ID (matches registry convention).
...
func (a *DiscoveryRegistryAdapter) UpdateAgentStatus(ctx context.Context, agentID string, status string) error {
	return a.service.Registry().UpdateAgentStatus(ctx, agentID, tools.AgentStatus(status))
}
