...
// --- In-memory mock of DBClient/DBRows ---
...
func (c *mockPGClient) Query(_ context.Context, query string, args ...any) (observation.DBRows, error) {
	c.mu.Lock()
	defer c.mu.Unlock()

	agentID := args[0].(string)
	var filtered []obsRecord
...
func TestPostgreSQLObservationStore_SaveAndLoadRecent(t *testing.T) {
	...
	store, err := observation.NewPostgreSQLObservationStore(ctx, client)
	require.NoError(t, err)

	now := time.Now().UTC().Truncate(time.Second)
	obs1 := observation.Observation{ID: "1", AgentID: "a1", Date: "2025-06-01", Content: "first", CreatedAt: now.Add(-2 * time.Hour)}
	obs2 := observation.Observation{ID: "2", AgentID: "a1", Date: "2025-06-01", Content: "second", CreatedAt: now.Add(-1 * time.Hour)}
	obs3 := observation.Observation{ID: "3", AgentID: "a2", Date: "2025-06-01", Content: "other agent", CreatedAt: now}
...
func TestPostgreSQLObservationStore_LoadByDateRange(t *testing.T) {
	...
	obs1 := observation.Observation{ID: "1", AgentID: "a1", Date: "2025-06-01", Content: "day1", CreatedAt: base}
	obs2 := observation.Observation{ID: "2", AgentID: "a1", Date: "2025-06-02", Content: "day2", CreatedAt: base.Add(24 * time.Hour)}
	obs3 := observation.Observation{ID: "3", AgentID: "a1", Date: "2025-06-03", Content: "day3", CreatedAt: base.Add(48 * time.Hour)}
...
func TestPostgreSQLObservationStore_Upsert(t *testing.T) {
	...
	now := time.Now().UTC().Truncate(time.Second)
	obs := observation.Observation{ID: "u1", AgentID: "a1", Date: "2025-06-01", Content: "original", CreatedAt: now}
	require.NoError(t, store.Save(ctx, obs))
...
func TestPostgreSQLObservationStore_NilDB(t *testing.T) {
	_, err := observation.NewPostgreSQLObservationStore(context.Background(), nil)
	assert.Error(t, err)
}
