...
type A2ATaskSender interface {
	SendTask(ctx context.Context, endpoint string, fromAgentID string, payload map[string]any) (any, error)
}

type TransportFactory func(ctx context.Context, target RemoteToolTarget) (mcpproto.Transport, error)
...
// NewDefaultRemoteToolTransport creates the default remote tool transport.
...
// Invoke dispatches a remote tool invocation to the target transport kind.
...
func (t *DefaultRemoteToolTransport) resolveMCPCaller(ctx context.Context, target RemoteToolTarget) (MCPToolCaller, func(), error) {
