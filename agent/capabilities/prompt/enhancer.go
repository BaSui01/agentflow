...
type PromptEnhancerConfig struct {
	UseChainOfThought   bool `json:"use_chain_of_thought"`
	UseSelfConsistency  bool `json:"use_self_consistency"`
...
func AddTaskDescription(prompt string) string {
	return fmt.Sprintf("请完成以下任务：\n\n%s", prompt)
}

func HasConstraints(prompt string) bool {
