...
| 文件存储 | 本地文件系统 + PG 元数据 | MVP 最简，后续可切 MinIO/S3 |
| MVP 范围 | 全功能 MVP | 6 Agent + DAG + RAG + HITL + 版本管理 + React 前端 |

## 已知信息（从 agentflow 代码库确认）

### agentflow 已有基础设施（直接复用，不重写）

| 能力 | agentflow 位置 | 复用方式 |
|------|---------------|---------|
| JWT + API Key 认证 | `cmd/agentflow/middleware.go` | 直接导入中间件 |
| 多租户（tenant_id 注入 + 限流） | `cmd/agentflow/middleware.go` + `types/context.go` | 导入 `WithTenantID/WithUserID` |
| PostgreSQL (GORM) | `internal/database/pool.go` | 导入 PoolManager |
| Redis 缓存 | `internal/cache/manager.go` | 导入 cache.Manager |
...
| HTTP Server + 中间件链 | `internal/server/` + `cmd/agentflow/middleware.go` | 导入 ServerManager + 中间件 |
| 配置热加载 | `config/hotreload.go` | 导入 HotReloadManager |
| OpenTelemetry + Prometheus | `internal/telemetry/` + `internal/metrics/` | 导入 |
...
| 可观测性 | OpenTelemetry + Prometheus (agentflow) | 已有完整集成 |
