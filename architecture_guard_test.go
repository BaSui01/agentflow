...
func TestDependencyDirectionGuards(t *testing.T) {
	...
	rules := []guardRule{
		...
		{sourcePrefix: "pkg", targetPrefix: "config", reason: "infrastructure pkg layer must not depend on global config; use self-contained config types instead"},
		...
		{sourcePrefix: "rag", targetPrefix: "config", reason: "RAG layer must not depend on global config package; use self-contained StoreConfig instead"},
