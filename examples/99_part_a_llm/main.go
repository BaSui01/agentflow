...
func mkTools(lg *zap.Logger) *tools.DefaultRegistry {
	...
	r.Register("translate", func(_ context.Context, a json.RawMessage) (json.RawMessage, error) {
		var p struct{ Text, To string `json:"text"` }; json.Unmarshal(a, &p)
...
func main() {
	lg, _ := zap.NewProduction(); defer lg.Sync()
...
func a01BasicCompletion(ctx context.Context, p llm.Provider) {
	...
	if e != nil { rec("基础Completion", "FAIL", time.Since(t), e.Error()); return }
	c := txt(r.Choices[0].Message)
	if strings.Contains(c, "2") { rec("基础Completion", "PASS", time.Since(t), cut(c, 50))
	} else { rec("基础Completion", "WARN", time.Since(t), cut(c, 80)) }
}
...
func a02BasicStream(ctx context.Context, p llm.Provider) {
	...
	if e != nil { rec("基础Stream", "FAIL", time.Since(t), e.Error()); return }
	var sb strings.Builder; n := 0
	for ck := range ch { if ck.Err != nil { rec("基础Stream", "FAIL", time.Since(t), ck.Err.Error()); return }; sb.WriteString(ck.Delta.Content); if ck.Delta.ReasoningContent != nil { sb.WriteString(*ck.Delta.ReasoningContent) }; n++ }
	if n > 0 { rec("基础Stream", "PASS", time.Since(t), fmt.Sprintf("%d chunks", n))
	} else { rec("基础Stream", "FAIL", time.Since(t), "0 chunks") }
}
...
func a03MultiTurn(ctx context.Context, p llm.Provider) {
	...
	r1, e := call(ctx, p, msgs); if e != nil { rec("多轮对话", "FAIL", time.Since(t), e.Error()); return }
	...
	r2, e := call(ctx, p, msgs); if e != nil { rec("多轮对话", "FAIL", time.Since(t), e.Error()); return }
	...
	} else { rec("多轮对话", "WARN", time.Since(t), cut(a, 80)) }
}
...
func a04ToolCallDetection(ctx context.Context, p llm.Provider) {
	...
	if e != nil { rec("ToolCall检测", "FAIL", time.Since(t), e.Error()); return }
	tc := r.Choices[0].Message.ToolCalls
	if len(tc) > 0 { rec("ToolCall检测", "PASS", time.Since(t), fmt.Sprintf("%d calls, tool=%s", len(tc), tc[0].Name))
	} else { rec("ToolCall检测", "WARN", time.Since(t), "未返回tool_calls") }
}
...
func a05ReActLoop(ctx context.Context, p llm.Provider, reg *tools.DefaultRegistry, lg *zap.Logger) {
	...
	if e != nil && resp == nil { rec("ReAct循环", "FAIL", time.Since(t), e.Error()); return }
	toolN := 0; for _, s := range steps { toolN += len(s.Actions) }
	if toolN > 0 { rec("ReAct循环", "PASS", time.Since(t), fmt.Sprintf("%d步 %d工具调用", len(steps), toolN))
	} else { rec("ReAct循环", "WARN", time.Since(t), fmt.Sprintf("%d步无工具", len(steps))) }
}
...
func a06StreamReAct(ctx context.Context, p llm.Provider, reg *tools.DefaultRegistry, lg *zap.Logger) {
	...
	if e != nil { rec("流式ReAct", "FAIL", time.Since(t), e.Error()); return }
	tc, hasC := 0, false
	for ev := range eCh {
		if ev.Type == tools.ReActEventToolsStart { tc += len(ev.ToolCalls) }
		if ev.Type == tools.ReActEventCompleted { hasC = true }
	}
	if tc > 0 { rec("流式ReAct", "PASS", time.Since(t), fmt.Sprintf("%d工具, completed=%v", tc, hasC))
	} else if hasC { rec("流式ReAct", "WARN", time.Since(t), "完成但无工具调用")
	} else { rec("流式ReAct", "FAIL", time.Since(t), "未完成") }
}
...
func a07StructuredJSON(ctx context.Context, p llm.Provider) {
	...
	if e != nil { rec("结构化JSON", "FAIL", time.Since(t), e.Error()); return }
	c := txt(r.Choices[0].Message)
	js := extractJSON(c)
	var m map[string]any
	if json.Unmarshal([]byte(js), &m) == nil && m["name"] != nil { rec("结构化JSON", "PASS", time.Since(t), cut(js, 80))
	} else { rec("结构化JSON", "FAIL", time.Since(t), cut(c, 80)) }
}
...
func a08PromptInjection(ctx context.Context, p llm.Provider) {
	...
	if e != nil { rec("Prompt注入防御", "FAIL", time.Since(t), e.Error()); return }
...
func a09LongContext(ctx context.Context, p llm.Provider) {
	...
	if e != nil { rec("长上下文", "FAIL", time.Since(t), e.Error()); return }
	a := txt(r.Choices[0].Message)
	if strings.Contains(a, secret) { rec("长上下文", "PASS", time.Since(t), "正确提取密码")
	} else { rec("长上下文", "WARN", time.Since(t), cut(a, 80)) }
}

func a10ConcurrentCalls(ctx context.Context, p llm.Provider) {
	t := time.Now(); n := 3; var wg sync.WaitGroup; var ok int64
	qs := []string{"1+1=?只答数字", "中国首都？只答城市名", "Go的吉祥物？只答名字"}
	for i := 0; i < n; i++ { wg.Add(1); go func(j int) { defer wg.Done()
		r, e := call(ctx, p, []types.Message{{Role: "user", Content: qs[j]}})
		if e == nil && len(r.Choices) > 0 { atomic.AddInt64(&ok, 1) }
	}(i) }
	wg.Wait(); s := int(atomic.LoadInt64(&ok))
	if s == n { rec("并发调用", "PASS", time.Since(t), fmt.Sprintf("%d/%d", s, n))
	} else if s > 0 { rec("并发调用", "WARN", time.Since(t), fmt.Sprintf("%d/%d", s, n))
	} else { rec("并发调用", "FAIL", time.Since(t), "全部失败") }
}
...
func a11ToolChainDependency(ctx context.Context, p llm.Provider, reg *tools.DefaultRegistry, lg *zap.Logger) {
	...
	_ = e
	ts := map[string]bool{}; for _, s := range steps { for _, a := range s.Actions { ts[a.Name] = true } }
	if ts["get_weather"] && ts["translate"] { rec("工具链依赖", "PASS", time.Since(t), fmt.Sprintf("%d步,调用%v", len(steps), mapKeys(ts)))
	} else if len(ts) > 0 { rec("工具链依赖", "WARN", time.Since(t), fmt.Sprintf("部分:%v", mapKeys(ts)))
	} else { rec("工具链依赖", "WARN", time.Since(t), "直接回答无工具") }
}
...
func a12ErrorRecovery(ctx context.Context, p llm.Provider, lg *zap.Logger) {
	...
	if resp != nil && txt(resp.Choices[0].Message) != "" { rec("错误恢复降级", "PASS", time.Since(t), "LLM降级回答")
	} else { rec("错误恢复降级", "WARN", time.Since(t), "无有效回答") }
}
...
func a13ToolTimeout(ctx context.Context, p llm.Provider, lg *zap.Logger) {
	...
	reg.Register("slow_api", func(c context.Context, _ json.RawMessage) (json.RawMessage, error) {
		select { case <-time.After(5 * time.Second): return json.Marshal("late"); case <-c.Done(): return nil, c.Err() }
	...
	if resp != nil && txt(resp.Choices[0].Message) != "" { rec("工具超时恢复", "PASS", time.Since(t), "超时后降级回答")
	} else { rec("工具超时恢复", "WARN", time.Since(t), "无有效回答") }
}
...
func a14LoopAgent(ctx context.Context, p llm.Provider) {
	...
	var outs []string
	for i := 0; i < 3; i++ {
		r, e := call(ctx, p, msgs); if e != nil { break }
		o := txt(r.Choices[0].Message); outs = append(outs, o)
		if strings.Contains(o, "DONE") { break }
		msgs = append(msgs, types.Message{Role: "assistant", Content: o}, types.Message{Role: "user", Content: "改进它，满意就加DONE"})
	}
	if len(outs) > 0 { rec("循环智能体", "PASS", time.Since(t), fmt.Sprintf("%d轮", len(outs)))
	} else { rec("循环智能体", "FAIL", time.Since(t), "无输出") }
}
...
func a15CoTReasoning(ctx context.Context, p llm.Provider) {
	...
	if e != nil { rec("CoT推理", "FAIL", time.Since(t), e.Error()); return }
	c := txt(r.Choices[0].Message)
	...
	} else { rec("CoT推理", "WARN", time.Since(t), cut(c, 80)) }
}
...
func a17RefusalDetection(ctx context.Context, p llm.Provider) {
	...
	if refused { rec("拒绝检测", "PASS", time.Since(t), "正确拒绝危险请求")
	} else { rec("拒绝检测", "FAIL", time.Since(t), "未拒绝！"+cut(c, 50)) }
}
...
func a18MultiLingual(ctx context.Context, p llm.Provider) {
	...
	if e != nil { rec("多语言", "FAIL", time.Since(t), e.Error()); return }
	c := txt(r.Choices[0].Message)
	// 检查是否包含日文字符
	hasJP := false; for _, ch := range c { if ch >= 0x3040 && ch <= 0x30FF || ch >= 0x4E00 && ch <= 0x9FFF { hasJP = true; break } }
	if hasJP { rec("多语言", "PASS", time.Since(t), cut(c, 50))
	} else { rec("多语言", "WARN", time.Since(t), cut(c, 50)) }
}
...
func a19SystemPromptAdherence(ctx context.Context, p llm.Provider) {
	...
	if e != nil { rec("系统指令遵循", "FAIL", time.Since(t), e.Error()); return }
	c := txt(r.Choices[0].Message)
	hasCN := false; for _, ch := range c { if ch >= 0x4E00 && ch <= 0x9FFF { hasCN = true; break } }
	if !hasCN { rec("系统指令遵循", "PASS", time.Since(t), "坚持英文回答")
	} else { rec("系统指令遵循", "WARN", time.Since(t), "使用了中文: "+cut(c, 50)) }
}
...
func a20LargeToolResponse(ctx context.Context, p llm.Provider, reg *tools.DefaultRegistry, lg *zap.Logger) {
	...
	bigReg.Register("big_data", func(context.Context, json.RawMessage) (json.RawMessage, error) {
		...
		for i := range items { items[i] = map[string]any{"id": i, "name": fmt.Sprintf("item_%d", i), "desc": strings.Repeat("x", 100)} }
	...
	if e != nil && resp == nil { rec("大工具响应", "WARN", time.Since(t), e.Error()); return }
	if resp != nil { rec("大工具响应", "PASS", time.Since(t), "大响应处理成功")
	} else { rec("大工具响应", "WARN", time.Since(t), "无响应") }
}
...
func printSummary() {
	...
	if fl == 0 { fmt.Println("\n  🎉 全部关键测试通过！") } else { fmt.Printf("\n  ⚠️  有 %d 项失败\n", fl) }
}

func extractJSON(s string) string {
	s = strings.TrimSpace(s)
	s = strings.TrimPrefix(s, "```json"); s = strings.TrimPrefix(s, "```"); s = strings.TrimSuffix(s, "```"); s = strings.TrimSpace(s)
	for i, ch := range s { if ch == '{' || ch == '[' { return s[i:] } }; return s
}
func mapKeys(m map[string]bool) []string { ks := make([]string, 0, len(m)); for k := range m { ks = append(ks, k) }; return ks }
