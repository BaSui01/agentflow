...
func demoEnhancedMemory(logger *zap.Logger) {
	...
	fmt.Println("--- 1. Enhanced Memory System ---")
	...
	sys := memory.NewDefaultEnhancedMemorySystem(config, logger)
	...
	_ = sys.SaveShortTerm(ctx, "demo-agent", "user asked about weather", map[string]any{"channel": "chat"})
	...
	_ = sys.RecordEpisode(ctx, &types.EpisodicEvent{
	...
	_ = sys.AddKnowledge(ctx, &memory.Entity{ID: "agentflow", Type: "project", Name: "AgentFlow"})



	_ = sys.AddKnowledgeRelation(ctx, &memory.Relation{FromID: "agentflow", ToID: "memory", Type: "supports", Weight: 1})



	_, _ = sys.QueryKnowledge(ctx, "agentflow")
	...
	obsStore := observation.NewInMemoryObservationStore(0)



	_ = obsStore.Save(ctx, observation.Observation{ID: "obs-1", AgentID: "demo-agent", Content: "observed behavior"})



	_, _ = obsStore.LoadRecent(ctx, "demo-agent", 10)
	...
	_ = coordinator.Save(ctx, "user asked weather", memorycore.MemoryWorking, map[string]any{"channel": "chat"})



	_ = coordinator.LoadRecent(ctx, memorycore.MemoryWorking, 10)
	...
	_, _ = ns.LoadRecent(ctx, "worker-a", memorycore.MemoryWorking, 5)
	...
	fmt.Println("  ✓ Enhanced memory initialized")
