...
description: "集成 Skill 到项目规范"
---

# 集成 Skill 到项目规范

将可复用的 Skill 适配并集成到你的项目开发规范中（不是直接集成到项目代码中）。
...
定位并读取 Skill 说明：
- 仓库中的 `.agents/skills/<skill-name>/SKILL.md`
- `AGENTS.md` 中的 Skill 列表（当在当前上下文中可用时）
...
如果此 Skill 经常使用，创建一个快捷 Skill：

```bash
$create-command use-<skill-name> 按照项目规范使用 <skill-name> Skill
```
